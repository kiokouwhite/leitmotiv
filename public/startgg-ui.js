...
(function () {
  ...
  let _lastSets           = []; // derniers sets chargés (repli pour la synchro score)

  // Auto-refresh
  const REFRESH_INTERVAL    = 30; // secondes — rafraîchissement de la liste des sets (lourd)
  const SCORE_SYNC_INTERVAL = 5;  // secondes — synchro légère du score du set lié (1 requête)
  ...
  // Transforme une erreur start.gg en message lisible (cas spécial : limite de requêtes 429)
  function startggErrMsg(err, prefix) {
    const s = String(err || '');
    if (s.indexOf('RATE_LIMIT') === 0) {
      const ra = parseInt(s.split(':')[1], 10);
      return '⏳ Trop de requêtes start.gg — réessai automatique'
        + (Number.isFinite(ra) && ra > 0 ? ` dans ~${ra}s` : ' sous peu');
    }
    return (prefix || 'Erreur : ') + s;
  }
  ...
  async function searchTournament(slugInputId, infoId, nameId, selectId) {
    ...
    if (data.error) { showStatus(startggErrMsg(data.error), true); return; }
    ...
    // Synchroniser le tournoi actif : persiste dans config.json + broadcast
    // 'tournamentConfigUpdate' à tous les clients. Sans ça l'indicateur du
    // header restait sur l'ancien tournoi (et les overlays aussi).
    // On ne touche PAS à eventId/phaseId : ils seront mis à jour quand
    // l'utilisateur sélectionne un bracket dans le sélecteur d'événements.
    ...
    }).catch(() => { /* sync best-effort, ne bloque pas l'UI */ });
    ...
    // Charger automatiquement les participants — évite le clic « Charger les participants ».
    // Sélectionne l'événement configuré si présent, sinon le premier de la liste.
    const evSel = selectId && document.getElementById(selectId);
    if (evSel && evSel.options.length) {
      const cfg = await fetch('/api/tournament-config').then(r => r.json()).catch(() => ({}));
      if (cfg.eventId && [...evSel.options].some(o => o.value === String(cfg.eventId))) {
        evSel.value = String(cfg.eventId);
      }
      await loadEvent(selectId);
    }
  }
  ...
  async function loadEvent(selectId) {
    ...
    await Promise.allSettled([fetchSets(eventId), fetchStreamQueue()]);
    startAutoRefresh();
  ...
  async function fetchEntrants(eventId, page) {
    ...
    // Callbacks secondaires (bracket/stats/h2h) : ne doivent JAMAIS faire échouer le chargement principal
    try { window.onStartggEntrantsLoaded?.(eventId, allEntrants); }
    catch (e) { console.warn('[startgg] onStartggEntrantsLoaded:', e); }
    try { window.onStartggEventLoaded?.(eventId); }
    catch (e) { console.warn('[startgg] onStartggEventLoaded:', e); }
  }
  ...
  function renderEntrants(list) {
    ...
    list.forEach(e => {
      ...
      row.innerHTML = `
        ...
        <div class="sgg-entrant-actions">
          <button class="btn btn-outline btn-sm sgg-apply-p1" data-tag="${e.prefix}" data-name="${e.tag}" data-seed="${e.seeding ?? ''}" data-player-id="${e.playerId ?? ''}" data-user-slug="${e.userSlug ?? ''}" data-entrant-id="${e.id ?? ''}">→ J1</button>
          <button class="btn btn-outline btn-sm sgg-apply-p2" data-tag="${e.prefix}" data-name="${e.tag}" data-seed="${e.seeding ?? ''}" data-player-id="${e.playerId ?? ''}" data-user-slug="${e.userSlug ?? ''}" data-entrant-id="${e.id ?? ''}">→ J2</button>
        </div>
      `;
    ...
    container.querySelectorAll('.sgg-apply-p1').forEach(btn => {
      btn.addEventListener('click', () => applyPlayer(1, btn.dataset.tag, btn.dataset.name, btn.dataset.seed ? parseInt(btn.dataset.seed) : null, btn.dataset.playerId ? parseInt(btn.dataset.playerId) : null, btn));
    });
    container.querySelectorAll('.sgg-apply-p2').forEach(btn => {
      btn.addEventListener('click', () => applyPlayer(2, btn.dataset.tag, btn.dataset.name, btn.dataset.seed ? parseInt(btn.dataset.seed) : null, btn.dataset.playerId ? parseInt(btn.dataset.playerId) : null, btn));
    });
  }
  ...
  window._sggApplySeq = window._sggApplySeq || { 1: 0, 2: 0 };

  function applyPlayer(playerNum, prefix, tag, seeding, playerId, btnEl) {
    ...
    if (playerId) {
      const myToken = ++window._sggApplySeq[playerNum];
      if (btnEl) btnEl.classList.add('sgg-loading');
      const ctrl = new AbortController();
      const timeoutId = setTimeout(() => ctrl.abort(), 8000);
      fetch(`/api/startgg/user/${playerId}/main?limit=3`, { signal: ctrl.signal })
        .then(r => r.json())
        .then(json => {
          clearTimeout(timeoutId);
          if (myToken !== window._sggApplySeq[playerNum]) return;
          if (json && json.character) {
            try {
              const ns = buildStateFromForm();
              ns[`player${playerNum}`].character = json.character;
              state[`player${playerNum}`].character = json.character;
              emitState(ns);
              if (typeof updateCharPreview === 'function') updateCharPreview(playerNum, json.character);
              if (typeof updateStockColorBtns === 'function') updateStockColorBtns(playerNum, json.character.name);
              showStatus(`Perso auto-détecté pour J${playerNum} : ${json.character.name}`);
            } catch (e) { console.warn('[sgg main] inject state:', e); }
          } else {
            console.log('[sgg main] aucun main détecté pour playerId', playerId, json?.reason);
          }
        })
        .catch(err => {
          clearTimeout(timeoutId);
          if (err.name !== 'AbortError') console.warn('[sgg main] fetch:', err);
        })
        .finally(() => { if (btnEl) btnEl.classList.remove('sgg-loading'); });
    }
  }
  ...
  function buildSetCard(s, roundFull, extraPrefix) {
    ...
    const stateBadge  = `<span class="sgg-state-badge" title="${SET_STATE_LABEL[state]||''}">${SET_STATE_EMOJI[state]||'•'}</span>`;
    ...
    const launchable = !!(e1Id && e2Id);   // set lançable = les deux joueurs déterminés
    const card = document.createElement('div');
    card.className = 'sgg-set-card' + (launchable ? '' : ' sgg-set-unlaunchable');
    card.innerHTML = `
  ...
  async function fetchSets(eventId) {
    try {
      ...
      const data = await res.json();
      if (data.error) { showStatus(startggErrMsg(data.error, 'Erreur sets : '), true); return; }
      _lastSets = data.sets?.nodes || [];
      renderSets(_lastSets);
      syncCurrentSetScore();
    } catch (_) {
      showStatus('Sets momentanément indisponibles — nouvel essai automatique', true);
    }
  }

  // Synchro start.gg → scoreboard : récupère le set lié au scoreboard DIRECTEMENT par son ID
  // (fonctionne quel que soit son état : en cours, terminé, ou au-delà de la pagination).
  async function syncCurrentSetScore() {
    const setId = document.getElementById('sgg-current-set-id')?.value;
    if (!setId) return; // aucun set appliqué au scoreboard

    let set = null;
    // 1) Précis : le set par son ID (gère set terminé / au-delà de la pagination).
    try {
      const res = await fetch(`/api/startgg/set/${encodeURIComponent(setId)}`);
      if (res.ok) {
        const j = await res.json();
        if (j && !j.error && Array.isArray(j.slots)) set = j;
      }
    } catch (_) {}
    // 2) Repli : si l'endpoint /set/:id est indispo (serveur non redémarré), réutiliser
    //    la dernière liste de sets déjà chargée — couvre le set en cours visible.
    if (!set) set = (_lastSets || []).find(s => String(s.id) === String(setId)) || null;
    if (!set || !Array.isArray(set.slots)) return;

    const [slot1, slot2] = set.slots;
    let sc1 = slot1?.standing?.stats?.score?.value;
    let sc2 = slot2?.standing?.stats?.score?.value;
    if (sc1 == null || sc2 == null) return;          // pas encore reporté sur start.gg
    sc1 = Math.max(0, sc1); sc2 = Math.max(0, sc2);  // DQ (-1) → 0
    if (sc1 === 0 && sc2 === 0) return;              // évite d'écraser un score en cours par 0-0

    // Respecter l'ordre des joueurs du scoreboard (gère le cas inversé)
    const p1EntrantId = document.getElementById('sgg-p1-entrant-id')?.value;
    const e2 = slot2?.entrant?.id;
    let p1score = sc1, p2score = sc2;
    if (p1EntrantId && String(e2) === String(p1EntrantId)) { p1score = sc2; p2score = sc1; }

    const cur1 = parseInt(document.getElementById('p1-score-display')?.textContent) || 0;
    const cur2 = parseInt(document.getElementById('p2-score-display')?.textContent) || 0;
    if (cur1 === p1score && cur2 === p2score) return; // déjà à jour

    if (typeof window.syncMatchScoreFromStartgg === 'function') {
      window.syncMatchScoreFromStartgg(p1score, p2score);
      showStatus(`Score synchronisé depuis start.gg : ${p1score}–${p2score}`);
    }
  }

  // SET_STATE : 1=créé, 2=en cours, 3=terminé, 6=appelé
  const SET_STATE_LABEL = { 1: 'En attente', 2: 'En cours', 3: 'Terminé', 6: 'Appelé' };
  const SET_STATE_EMOJI = { 1: '⏳', 2: '🟢', 3: '✅', 6: '🔔' };  // ⏳ pas lancé · 🟢 lancé · ✅ fini · 🔔 appelé
    ...
    phases.forEach(phase => {
      ...
      if (!roundOrder.length) {
        ...
        roundOrder.forEach(rg => {
          ...
          // Round « vide » = aucun set actif n'a ses deux joueurs déterminés (rien de lançable)
          const roundHasLaunchable = (rg.active || []).some(s => {
            const sl = s.slots || [];
            return !!(sl[0]?.entrant?.id && sl[1]?.entrant?.id);
          });
          const emptyRoundCls = roundHasLaunchable ? '' : ' sgg-round-empty';

          // Bouton de round
          const roundBtn = document.createElement('div');
          roundBtn.className = 'sgg-round-btn' + (rIsOpen ? ' open' : '') + emptyRoundCls;
          ...
          roundList.className = 'sgg-round-sets' + (rIsOpen ? ' open' : '') + emptyRoundCls;
  ...
  function startAutoRefresh() {
    ...
    // Synchro légère du score du set lié, plus fréquente que la liste des sets
    _scoreSyncTimer = setInterval(() => { syncCurrentSetScore(); }, SCORE_SYNC_INTERVAL * 1000);
  ...
  // ── Pause des requêtes start.gg quand l'onglet est en arrière-plan ──────────────
  // Quand le panneau n'est pas l'onglet visible (tu regardes OBS/le jeu), on arrête
  // toutes les minuteries pour ne pas consommer le quota start.gg. Reprise au retour.
  function stopAutoRefresh() {
    if (_refreshTimer)   { clearInterval(_refreshTimer);   _refreshTimer = null; }
    if (_countdownTimer) { clearInterval(_countdownTimer); _countdownTimer = null; }
    if (_scoreSyncTimer) { clearInterval(_scoreSyncTimer); _scoreSyncTimer = null; }
    updateCountdown(0);
  }
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      ...
      // Retour au premier plan : rattrapage immédiat puis relance des minuteries
      doRefresh();
      syncCurrentSetScore();
      startAutoRefresh();
    }
  ...
  async function autoLoadFromConfig() {
    try {
      ...
      // Renseigner les champs slug cachés des deux panneaux
      ['sgg-slug', 'match-sgg-slug'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = cfg.slug;
      });

      // Aucun bracket choisi : lancer la recherche (remplit la liste + charge le 1er événement)
      if (!cfg.eventId) {
        await searchTournament('match-sgg-slug', 'match-sgg-tournament-info', 'match-sgg-tournament-name', 'match-sgg-event-select');
        return;
      }
  ...
  // Participants du tournoi courant (pour le chargement de casters)
  window.sggGetEntrants = () => allEntrants || [];
  autoLoadFromConfig();

  // Personnages du jeu de l'event courant (pour la fenêtre de report game data).
  // Mise en cache par eventId pour éviter de re-télécharger.
  const _charsCache = {};
  window.sggGetCharacters = async function () {
    if (!currentEventId) return [];
    if (_charsCache[currentEventId]) return _charsCache[currentEventId];
    try {
      const res  = await fetch(`/api/startgg/event/${currentEventId}/characters`);
      const data = await res.json();
      const chars = Array.isArray(data.characters) ? data.characters : [];
      _charsCache[currentEventId] = chars;
      return chars;
    } catch (_) { return []; }
  };
  ...
  window.onStartggEventLoaded = function(evId) {
    ...
    const bracketSec = document.getElementById('sgg-bracket-section');
    if (bracketSec) bracketSec.style.display = '';
    const top8Sec = document.getElementById('sgg-top8-section');
    if (top8Sec) top8Sec.style.display = '';
...
// ── Filtre « sets lançables » : bouton bas de la modale start.gg ──────────────
// Masque les sets dont les deux joueurs ne sont pas encore déterminés (TBD),
// donc impossibles à lancer. La classe est posée sur la modale → survit aux
// re-rendus automatiques de la liste (les cartes non-lançables sont re-marquées).
(function () {
  const KEY = 'sgg-hide-unlaunchable';
  const btn = document.getElementById('sgg-toggle-unlaunchable');
  const modal = document.getElementById('sgg-picker-modal');
  if (!btn || !modal) return;
  function apply(on) {
    modal.classList.toggle('hide-unlaunchable', on);
    btn.classList.toggle('active', on);
    btn.textContent = on ? 'Afficher tous les sets' : 'Cacher les sets non lançables';
  }
  let on = false;
