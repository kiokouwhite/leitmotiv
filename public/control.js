...
let state = {
  ...
  format: 'Bo3',
  customWins: '',   // format « custom » = texte libre (ex : BO7, FT5, Money Match)
  scoreboardLayout: 'classic', // preset de disposition du scoreboard (Customisation > SSBU > Scoreboard > Presets)
  event: 'TOURNAMENT',
  stage: 'Grand Final',
  visible: true,
};
...
function syncFromState(s) {
  ...
  const _logoToggle = document.getElementById('btn-logo-toggle');
  if (_logoToggle) {
    const logoHidden = !!s.centerLogoHidden;
  ...
  // Compat : ancienne case sbSquashText (booleen) -> mode 'squash'.
  const _ovM = document.getElementById('sb-overflow-mode');
  if (_ovM) _ovM.value = s.sbOverflowMode || (s.sbSquashText === true ? 'squash' : 'grow');
  ...
  [
    ...
    if (cb) cb.checked = (s[field] !== false);
  });
  // Lot 2 : logo central enrichi + écart entre cartes — restaure les contrôles depuis l'état
  [
    ...
    // Lot 3 — sliders géométrie/tailles (idBase + state field + default)
    ['player-min-width',          'playerCardMinWidth',      320],
    ...
    ['pronouns-font-size',        'pronounsFontSize',        14],
    ['seed-font-size',            'seedFontSize',            11],
  ...
  const shapeSel = document.getElementById('center-logo-shape');
  if (shapeSel) shapeSel.value = s.centerLogoShape || 'none';
  const glowCol = document.getElementById('center-logo-glow-color');
  if (glowCol && s.centerLogoGlowColor) glowCol.value = s.centerLogoGlowColor;
  // Lot 3 — couleurs P1/P2 + ombre + position score + miroir
  const p1Col = document.getElementById('p1-accent-color');
  if (p1Col && s.player1?.color) p1Col.value = s.player1.color;
  const p2Col = document.getElementById('p2-accent-color');
  ...
  [
    ...
    ['eb-right-width', 'eventBarRightWidth', 0],
  ].forEach(([idBase, field, def]) => {
    const v = (s[field] != null) ? s[field] : def;
    const rng = document.getElementById(idBase + '-range');
    const num = document.getElementById(idBase + '-num');
  ...
  const ebAlign = s.eventBarLeftAlign || 'left';
  document.querySelectorAll('.eb-left-align-btn').forEach(b => b.classList.toggle('active', b.dataset.align === ebAlign));
  const ebStack = s.eventStacking || 'inline';
  ...
  [
    ...
    ['event-bar-radius',        'eventBarRadius',        0],
...
function buildStateFromForm() {
  return {
    ...
    scoreboardLayout: state.scoreboardLayout || 'classic',
    ...
    sbOverflowMode:      document.getElementById('sb-overflow-mode')?.value || 'grow',
    ...
    showFormat:     document.getElementById('show-format')?.checked !== false,
    showRound:      document.getElementById('show-round')?.checked !== false,
    showFlags:      document.getElementById('show-flags')?.checked !== false,
    showPronouns:   document.getElementById('show-pronouns')?.checked !== false,
    ...
    centerLogoOpacity:       parseInt(document.getElementById('center-logo-opacity-num')?.value ?? 100),
    centerLogoShape:         document.getElementById('center-logo-shape')?.value || 'none',
    centerLogoGlowColor:     document.getElementById('center-logo-glow-color')?.value || '#00f0ff',
    centerLogoGlowIntensity: parseInt(document.getElementById('center-logo-glow-num')?.value ?? 0),
    playersGap:              parseInt(document.getElementById('players-gap-num')?.value ?? 0),
    // Lot 3 : géométrie des cartes joueur
    playerCardMinWidth:  parseInt(document.getElementById('player-min-width-num')?.value ?? 320),
    ...
    pronounsFontSize:    parseInt(document.getElementById('pronouns-font-size-num')?.value ?? 14),
    seedFontSize:        parseInt(document.getElementById('seed-font-size-num')?.value ?? 11),
    ...
    eventBarLeftAlign:   document.querySelector('.eb-left-align-btn.active')?.dataset.align || 'left',
    ...
    eventBarRadius:      parseInt(document.getElementById('event-bar-radius-num')?.value ?? 0),
...
/* ── Application automatique (plus de bouton « Appliquer ») ───── */
/* Les modifs du scoreboard sont poussées ~1 s après la dernière saisie. */
let _autoApplyTimer    = null;
let _autoApplyInterval = null;
const BTN_APPLY        = document.getElementById('btn-apply'); // null : bouton retiré (les usages sont protégés)
const AUTO_APPLY_DELAY = 1; // secondes
...
// Exposé pour startgg-ui.js : met à jour le score ET pousse vers l'overlay
// (utilisé par la synchro auto start.gg → scoreboard)
window.syncMatchScoreFromStartgg = function(p1score, p2score) {
  window.setMatchScore(p1score, p2score);
  emitState(buildStateFromForm());
};

// Le bouton « Envoyer le score » ouvre désormais la fenêtre de saisie des games
document.getElementById('btn-send-startgg').addEventListener('click', () => openReportModal());

// ── Fenêtre de report game data (gagnant + personnages par game) ─────────────────
(function () {
  const modal      = document.getElementById('report-modal');
  if (!modal) return;
  const gamesList  = document.getElementById('report-games-list');
  const matchLabel = document.getElementById('report-match-label');
  ...
  let ctx = null;        // { setId, p1EntrantId, p2EntrantId }
  let nameToId = {};     // nom (minuscule) → id start.gg du personnage

  // Liste de suggestions partagée (recherche au clavier dans les champs perso)
  let charDatalist = document.getElementById('report-char-list');
  if (!charDatalist) {
    charDatalist = document.createElement('datalist');
    charDatalist.id = 'report-char-list';
    modal.appendChild(charDatalist);
  }
  ...
  function resolveCharId(val) {
    const n = (val || '').trim().toLowerCase();
    return n && nameToId[n] ? nameToId[n] : null;
  }
  function buildGameRow(num, winner, p1Label, p2Label) {
    const row = document.createElement('div');
    row.className = 'report-game';
    row.dataset.game = num;
    row.innerHTML =
      `<div class="report-game-head">Game ${num} <span class="report-game-hint">— clique le gagnant</span></div>` +
      `<div class="report-player-line">` +
        `<button type="button" class="report-win-btn${winner === 1 ? ' active' : ''}" data-winner="1">${esc(p1Label)}</button>` +
        `<input class="report-char report-char-p1" list="report-char-list" placeholder="— Perso —" autocomplete="off" />` +
      `</div>` +
      `<div class="report-player-line">` +
        `<button type="button" class="report-win-btn${winner === 2 ? ' active' : ''}" data-winner="2">${esc(p2Label)}</button>` +
        `<input class="report-char report-char-p2" list="report-char-list" placeholder="— Perso —" autocomplete="off" />` +
      `</div>`;
    row.querySelectorAll('.report-win-btn').forEach(b => {
      b.addEventListener('click', () => {
        row.querySelectorAll('.report-win-btn').forEach(x => x.classList.remove('active'));
  ...
  // Propagation : choisir un perso valide remplit les autres games VIDES du même joueur
  function propagateChar(e) {
    const inp = e.target.closest('.report-char');
    if (!inp) return;
    const val = inp.value.trim();
    if (!val || !resolveCharId(val)) return;            // seulement un perso complet/valide
    const player = inp.classList.contains('report-char-p2') ? 2 : 1;
    gamesList.querySelectorAll('.report-char-p' + player).forEach(o => {
      if (o !== inp && !o.value.trim()) o.value = inp.value;
    });
  }
  gamesList.addEventListener('input', propagateChar);
  gamesList.addEventListener('change', propagateChar);

  window.openReportModal = async function () {
    const sb = document.getElementById('btn-send-startgg');
    const setId       = document.getElementById('sgg-current-set-id')?.value || sb?.dataset.setid       || '';
    ...
    const p2Label = (document.getElementById('p2-name')?.value || '').trim() || 'J2';
    ctx = { setId, p1EntrantId, p2EntrantId };

    if (matchLabel) matchLabel.textContent = `${p1Label} vs ${p2Label}`;
    setReportStatus('');
    ...
    let chars = [];
    try { chars = (typeof window.sggGetCharacters === 'function') ? await window.sggGetCharacters() : []; } catch (_) {}

    // Construire la liste de suggestions (recherche clavier) + la table nom→id
    nameToId = {};
    let dl = '';
    chars.forEach(c => { nameToId[String(c.name).toLowerCase()] = c.id; dl += `<option value="${esc(c.name)}"></option>`; });
    charDatalist.innerHTML = dl;
    ...
    if (total === 0) {
      ...
      for (let i = 0; i < total; i++) {
        const winner = i < p1Score ? 1 : 2; // pré-remplissage cohérent avec le score
        gamesList.appendChild(buildGameRow(i + 1, winner, p1Label, p2Label));
      }
    }
  ...
  sendBtn?.addEventListener('click', async () => {
    if (!ctx) return;
    const rows = [...gamesList.querySelectorAll('.report-game')];
    if (!rows.length) return;
    const games = rows.map(r => ({
      winner:   r.querySelector('.report-win-btn.active')?.dataset.winner === '2' ? 2 : 1,
      p1CharId: resolveCharId(r.querySelector('.report-char-p1')?.value),
      p2CharId: resolveCharId(r.querySelector('.report-char-p2')?.value),
    }));
    const p1wins = games.filter(g => g.winner === 1).length;
    if (p1wins === games.length - p1wins) { setReportStatus('Égalité de games — désigne un vainqueur', 'err'); return; }

    sendBtn.disabled = true;
    setReportStatus('Envoi…');
    try {
      const res = await fetch(`/api/startgg/set/${ctx.setId}/report`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ p1EntrantId: ctx.p1EntrantId, p2EntrantId: ctx.p2EntrantId, games }),
      });
      const data = await res.json();
      if (data.error) { setReportStatus('Erreur start.gg : ' + data.error, 'err'); sendBtn.disabled = false; return; }
      setReportStatus('Score envoyé !', 'ok');
      // Set terminé : on vide la liaison et on masque le bouton d'envoi
      const sb = document.getElementById('btn-send-startgg');
      ...
      setReportStatus('Erreur : ' + e.message, 'err');
...
// ── Sélecteur de match start.gg (modale) ────────────────────────────────────────
// Ouvre les sections « connexion » + « sets start.gg » dans une fenêtre, depuis le
// panneau Informations du match (au lieu d'aller en bas de page).
(function () {
  const modal   = document.getElementById('sgg-picker-modal');
  const body    = document.getElementById('sgg-picker-body');
  const openBtn = document.getElementById('btn-open-sgg-picker');
  const closeBtn = document.getElementById('sgg-picker-close');
  if (!modal || !body || !openBtn) return;

  let moved = false;
  function ensureMoved() {
    if (moved) return;
    const connect = document.getElementById('match-sgg-connect-section');
    const sets    = document.getElementById('match-sgg-sets-section');
    if (connect) body.appendChild(connect);
    if (sets)    body.appendChild(sets);
    moved = true;
  }

  // Déplacer tout de suite dans la fenêtre pour que les sets n'apparaissent plus en bas de page
  ensureMoved();
  ...
  openBtn.addEventListener('click', () => {
    ...
    const sets    = document.getElementById('match-sgg-sets-section');
    const connect = document.getElementById('match-sgg-connect-section');
    // Toujours offrir une action : si aucun set affiché, montrer le bloc de connexion/recherche
    const setsVisible = sets && getComputedStyle(sets).display !== 'none';
    if (connect && !setsVisible) connect.style.display = '';
    modal.style.display = 'flex';
  });

  closeBtn?.addEventListener('click', () => { modal.style.display = 'none'; });
  modal.addEventListener('click', (e) => { if (e.target === modal) modal.style.display = 'none'; });
  // Choisir un match (appliquer un set) ferme la fenêtre
  modal.addEventListener('click', (e) => {
    if (e.target.closest('.sgg-apply-set')) setTimeout(() => { modal.style.display = 'none'; }, 150);
  });
})();
...
document.getElementById('btn-swap').addEventListener('click', () => {
  ...
  swapVal('p1-social-1',  'p2-social-1');
  swapVal('p1-social-2',  'p2-social-2');
  swapVal('p1-social-3',  'p2-social-3');
  swapText('p1-score-display', 'p2-score-display');

  // ── Swap aussi la liaison start.gg (IDs d'entrants) pour rester aligné J1/J2 ──
  // Sinon, après inversion, le gagnant est envoyé au mauvais joueur sur start.gg.
  swapVal('sgg-p1-entrant-id', 'sgg-p2-entrant-id');
  const _sb = document.getElementById('btn-send-startgg');
  if (_sb) {
    const _te = _sb.dataset.p1entrantid;
    _sb.dataset.p1entrantid = _sb.dataset.p2entrantid || '';
    _sb.dataset.p2entrantid = _te || '';
  }
...
// VS Screen : toggle ON/OFF (afficher = animation d'entrée, masquer = animation de sortie).
let _vsVisible = false;
function updateVsToggleBtn(visible) {
  _vsVisible = visible;
  const btn = document.getElementById('btn-vs-trigger');
  if (btn) {
    btn.textContent = visible ? '⚔ VS Screen : Affiché' : '⚔ VS Screen : Masqué';
    btn.style.color = visible ? 'var(--gold)' : '';
    btn.style.borderColor = visible ? 'var(--gold)' : '';
  }
}
document.getElementById('btn-vs-trigger')?.addEventListener('click', () => {
  if (_vsVisible) {
    socket.emit('hideVsScreen');      // → vsScreenHide (animation de sortie)
    updateVsToggleBtn(false);
  } else {
    socket.emit('triggerVsScreen');   // → vsScreenTrigger (animation d'entrée)
    updateVsToggleBtn(true);
  }
});

// Poignées flottantes (gauche + droite) : masquer/afficher toutes les barres de nav.
// Alignées verticalement sur la 1re ligne de nav (la barre d'onglets du haut),
// clampées sous le header (sticky) quand on scrolle pour rester accessibles.
(function () {
  const btns = [document.getElementById('btn-toggle-navs'),
                document.getElementById('btn-toggle-navs-left')].filter(Boolean);
  const app = document.querySelector('.app');
  if (!btns.length || !app) return;
  const navSel = '.tab-nav, .tab-nav-mobile, .themes-subnav, .match-subnav, .match-subpanel-nav, .genov-subnav';

  function reposition() {
    const header = document.querySelector('.header');
    const headerBottom = header ? header.getBoundingClientRect().bottom : 0;
    // 1re barre de nav visible (en ordre DOM = la barre d'onglets du haut)
    const navs = [...app.querySelectorAll(navSel)].filter(n => n.offsetParent !== null);
    let centerY;
    if (navs.length) {
      const r = navs[0].getBoundingClientRect();
      centerY = r.top + r.height / 2;
    } else {
      centerY = headerBottom + 16;   // barres cachées : juste sous le header
    }
    centerY = Math.max(headerBottom + 16, centerY);   // jamais au-dessus du header
    btns.forEach(b => { b.style.top = centerY + 'px'; });
  }
  window.__repositionNavToggle = reposition;   // appelée aussi depuis switchTab

  function toggle() {
    const hidden = app.classList.toggle('navs-hidden');
    btns.forEach(b => {
      b.textContent = hidden ? '▼' : '▲';
      b.title = hidden ? 'Afficher les barres de navigation' : 'Masquer les barres de navigation';
    });
    reposition();
    window.dispatchEvent(new Event('resize'));   // recalcule le zoom du scoreboard (hauteur dispo changée)
  }
  btns.forEach(b => b.addEventListener('click', toggle));

  let ticking = false;
  const onSR = () => { if (ticking) return; ticking = true; requestAnimationFrame(() => { ticking = false; reposition(); }); };
...
// (Bouton « ↺ Serveur » retiré : redémarrage via start.bat, ou « npm run dev »/nodemon en dev.)
...
function switchTab(tabId) {
  ...
  setTimeout(() => { window.__repositionNavToggle && window.__repositionNavToggle(); window.dispatchEvent(new Event('resize')); }, 0);
}
...
// ── Onglet Customisation : sous-nav Thème / Scoreboard / Caster ──────────
// On déplace dans cet onglet : le panneau « Customisation du scoreboard »
// (→ Scoreboard) et le Roster + la Disposition des casters (→ Caster).
(function () {
  const tab = document.getElementById('tab-themes');
  if (!tab) return;

  // → Scoreboard : panneau de customisation du scoreboard
  const paramsPane = document.getElementById('themes-pane-params');
  const custom = document.getElementById('sb-sub-custom');
  if (paramsPane && custom) {
    paramsPane.appendChild(custom);
    custom.classList.add('active'); // .match-subpanel.active → visible (la pane parente gère le show/hide)
  }

  // → Next Match : panneau « Next Match » (OBS / Cadre / Éléments + aperçu) déplacé en entier
  const nmPane = document.getElementById('themes-pane-nextmatch');
  const nmPanel = document.getElementById('nm-sub-match');
  if (nmPane && nmPanel) {
    nmPane.appendChild(nmPanel);
    nmPanel.classList.add('active'); // .match-subpanel.active → visible (la pane parente gère le show/hide)
  }

  // → VS Screen : panneau « VS Screen » (OBS / Fond / Effets / Particules / Animation + aperçu) déplacé en entier
  const vsPane = document.getElementById('themes-pane-vsscreen');
  const vsPanel = document.getElementById('vs-sub-main');
  if (vsPane && vsPanel) {
    vsPane.appendChild(vsPanel);
    vsPanel.classList.add('active'); // .match-subpanel.active → visible (la pane parente gère le show/hide)
  }

  // → Victory : panneau « Overlay Victoire » (OBS / Fond / Effets / Particules / Animation + aperçu) déplacé en entier
  const vicPane = document.getElementById('themes-pane-victory');
  const vicPanel = document.getElementById('vic-sub-main');
  if (vicPane && vicPanel) {
    vicPane.appendChild(vicPanel);
    vicPanel.classList.add('active'); // .match-subpanel.active → visible (la pane parente gère le show/hide)
  }

  // → Caster : Roster + cartes (Disposition/Fond/Éléments) réparties en sections + preview à droite
  if (document.getElementById('themes-pane-caster')) {
    const preview    = document.getElementById('preview-wrap-casters');
    const previewBox = preview ? preview.parentElement : null;
    const roster     = document.querySelector('#casters-sub-match .caster-roster-section');
    const move = (srcId, dstId) => { const s = document.getElementById(srcId), d = document.getElementById(dstId); if (s && d) d.appendChild(s); };
    if (preview && document.getElementById('caster-preview-slot')) { preview.style.marginBottom = ''; document.getElementById('caster-preview-slot').appendChild(preview); }
    if (previewBox && !previewBox.children.length) previewBox.remove(); // retire le conteneur 60% vidé
    if (roster && document.getElementById('caster-sect-roster')) { roster.style.marginBottom = ''; document.getElementById('caster-sect-roster').appendChild(roster); }
    move('cc-dispo', 'caster-sect-dispo');
    move('cc-fond', 'caster-sect-fond');
    move('cc-elements', 'caster-sect-elements');
  }

  // → Générique : nav + panneaux des overlays génériques déplacés depuis « Principal »
  (function () {
    const src = document.getElementById('tab-overlays');
    const dst = document.getElementById('cust-cat-generique');
    if (src && dst) { while (src.firstChild) dst.appendChild(src.firstChild); }
  })();

  // ── Navigation à 2 niveaux : catégories (Thème / SSBU / Générique) + sous-onglets SSBU ──
  const ssbuCat   = document.getElementById('cust-cat-ssbu');
  const genCat    = document.getElementById('cust-cat-generique');
  const themePane = document.getElementById('themes-pane-theme');
  const ssbuPaneIds = ['themes-pane-params', 'themes-pane-caster', 'themes-pane-nextmatch', 'themes-pane-vsscreen', 'themes-pane-victory'];

  // Mise en page « onglet Principal » : nav niveau-1 (Thème/SSBU/Générique) et nav niveau-2
  // sur la MÊME ligne. On extrait les navs niveau-2 de leur conteneur pour les poser à côté
  // de la nav niveau-1 dans une .match-nav-row ; leur visibilité est pilotée par showCat().
  const catNav  = document.getElementById('cust-cat-nav');
  const ssbuNav = document.getElementById('cust-ssbu-nav');
  const modeNav = document.getElementById('cust-mode-nav');
  const genNav  = genCat ? genCat.querySelector('.genov-subnav') : null;
  if (catNav && catNav.parentNode) {
    const row = document.createElement('div');
    row.className = 'match-nav-row';
    row.id = 'cust-nav-row';
    catNav.parentNode.insertBefore(row, catNav);
    row.appendChild(catNav);
    if (ssbuNav) row.appendChild(ssbuNav);
    if (modeNav) row.appendChild(modeNav);
    if (genNav)  row.appendChild(genNav);
  }

  // Niveau 3 (par overlay SSBU) : Paramètres (réglages existants) | Presets (dispositions préfaites).
  const OVERLAY_LABEL = {
    'themes-pane-params': 'Scoreboard', 'themes-pane-caster': 'Caster',
    'themes-pane-nextmatch': 'Next Match', 'themes-pane-vsscreen': 'VS Screen', 'themes-pane-victory': 'Victory'
  };
  let currentOverlay = 'themes-pane-params'; // overlay SSBU actif
  let currentMode = 'params';                 // 'params' | 'presets'
  ...
  function renderPresets(overlayId) {
    ...
    if (!grid) return;
    if (overlayId === 'themes-pane-params') {
  ...
  function applySsbuView() {
    ...
    ssbuPaneIds.forEach(p => { const el = document.getElementById(p); if (el) el.style.display = (inParams && p === currentOverlay) ? '' : 'none'; });
    const pp = document.getElementById('themes-pane-presets');
    if (pp) pp.style.display = inParams ? 'none' : '';
    if (!inParams) renderPresets(currentOverlay);
    document.querySelectorAll('#cust-ssbu-nav .cust-ssbu-btn').forEach(b => b.classList.toggle('active', b.dataset.pane === currentOverlay));
    document.querySelectorAll('#cust-mode-nav .cust-mode-btn').forEach(b => b.classList.toggle('active', b.dataset.mode === currentMode));
  }

  function showSsbuPane(id) { currentOverlay = id; applySsbuView(); }
  showSsbuPane('themes-pane-params'); // overlay SSBU par défaut

  function showCat(cat) {
    document.querySelectorAll('#cust-cat-nav .cust-cat-btn').forEach(b => b.classList.toggle('active', b.dataset.cat === cat));
    if (themePane) themePane.style.display = (cat === 'theme') ? '' : 'none';
    if (ssbuCat)   ssbuCat.style.display   = (cat === 'ssbu') ? '' : 'none';
    if (genCat)    genCat.style.display    = (cat === 'generique') ? '' : 'none';
    // navs déplacées sur la ligne de la nav niveau-1 : n'afficher que les bonnes
    if (ssbuNav) ssbuNav.style.display = (cat === 'ssbu') ? '' : 'none';
    if (modeNav) modeNav.style.display = (cat === 'ssbu') ? '' : 'none';
    if (genNav)  genNav.style.display  = (cat === 'generique') ? '' : 'none';
    setTimeout(() => window.dispatchEvent(new Event('resize')), 0);
  }
  document.querySelectorAll('#cust-cat-nav .cust-cat-btn').forEach(btn => {
    btn.addEventListener('click', () => showCat(btn.dataset.cat));
  });
  document.querySelectorAll('#cust-ssbu-nav .cust-ssbu-btn').forEach(btn => {
    btn.addEventListener('click', () => { showSsbuPane(btn.dataset.pane); setTimeout(() => window.dispatchEvent(new Event('resize')), 0); });
  });
  document.querySelectorAll('#cust-mode-nav .cust-mode-btn').forEach(btn => {
    btn.addEventListener('click', () => { currentMode = btn.dataset.mode; applySsbuView(); setTimeout(() => window.dispatchEvent(new Event('resize')), 0); });
  });
  showCat('theme'); // état initial : Thème actif → navs niveau-2/3 masquées
})();

// ── Customisation : aperçu « Master Overlay » sous chaque aperçu en direct ──
// Montre le rendu composite (/master) en plus de l'aperçu de l'overlay édité.
// (Le chargement/déchargement est géré par le gestionnaire d'aperçus ci-dessous.)
// Une checkbox dans chaque label permet d'activer/désactiver TOUS les aperçus
// Master Overlay (Scoreboard, Casters, Next Match, VS Screen, Victory) en même
// temps. Préférence persistée dans localStorage. Quand un mwrap est display:none,
// la frame est automatiquement déchargée par le gestionnaire d'aperçus ci-dessous
// (ResizeObserver → width=0 → src=about:blank).
(function () {
  const STORAGE_KEY = 'pso_master_preview_enabled';
  let enabled = localStorage.getItem(STORAGE_KEY);
  enabled = (enabled === null) ? true : (enabled === '1');

  const items = []; // pour synchroniser les checkboxes entre eux

  ['preview-wrap-scoreboard', 'preview-wrap-casters', 'preview-wrap-nextmatch', 'preview-wrap-vsscreen', 'preview-wrap-victory'].forEach(id => {
    const wrap = document.getElementById(id);
    if (!wrap || !wrap.after) return;

    // Label cliquable avec checkbox : « ☐/☑ Preview Master Overlay »
    const label = document.createElement('label');
    label.style.cssText = 'display:flex;align-items:center;gap:8px;font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.08em;margin-top:14px;margin-bottom:8px;cursor:pointer;user-select:none';
    const cb = document.createElement('input');
    cb.type = 'checkbox';
    cb.checked = enabled;
    cb.style.cssText = 'cursor:pointer;margin:0';
    const txt = document.createElement('span');
    txt.textContent = 'Preview Master Overlay';
    label.append(cb, txt);

    const mwrap = document.createElement('div');
    mwrap.className = 'overlay-preview-wrap';
    mwrap.style.cssText = 'border-radius:8px;overflow:hidden;background:#000;box-shadow:0 4px 24px rgba(0,0,0,0.5)';
    if (!enabled) mwrap.style.display = 'none';
    const frame = document.createElement('iframe');
    frame.className = 'overlay-preview-frame';
    frame.dataset.src = '/master';
    mwrap.appendChild(frame);
    wrap.after(label, mwrap);
    ...
    cb.addEventListener('change', () => {
      ...
      // Synchroniser TOUTES les checkboxes et tous les wraps (les 5 previews Master).
      items.forEach(it => {
        it.cb.checked = enabled;
        it.mwrap.style.display = enabled ? '' : 'none';
      });
      // Réveille le gestionnaire d'aperçus (charge / décharge la frame selon la nouvelle taille).
      window.dispatchEvent(new Event('resize'));
    });
  });
})();

// ── Aperçus en direct : ne charger que ceux visibles, décharger les autres ──
// Chaque aperçu est une page overlay complète (socket.io + polices + animations) :
// les charger tous en même temps sature le navigateur. On les (dé)charge selon leur
// visibilité, détectée par ResizeObserver (taille → 0 quand caché, > 0 quand affiché ;
// fiable même hors zone visible, contrairement à IntersectionObserver).
(function () {
  const frames = [
    ...document.querySelectorAll('.overlay-preview-frame'),
    document.getElementById('frames-preview'),
    document.getElementById('title-preview'),
    document.getElementById('ticker-preview'),
  ].filter(Boolean);
  // Normalise : tout en data-src (rien ne se charge tant que ce n'est pas visible).
  frames.forEach(fr => {
    if (!fr.dataset.src && fr.getAttribute('src')) { fr.dataset.src = fr.getAttribute('src'); fr.removeAttribute('src'); }
  });

  if (!('ResizeObserver' in window)) {
    frames.forEach(fr => { if (fr.dataset.src) fr.src = fr.dataset.src; }); // sans RO : on charge tout
    return;
  }
  function load(fr)   { if (fr.__pfOn || !fr.dataset.src) return; fr.src = fr.dataset.src; fr.__pfOn = true; }
  function unload(fr) { if (!fr.__pfOn) return; fr.src = 'about:blank'; fr.__pfOn = false; } // coupe socket + rendu
  const ro = new ResizeObserver(entries => {
    entries.forEach(e => {
      const fr = e.target.__pfFrame;
      if (!fr) return;
      if (e.contentRect.width > 1) load(fr); else unload(fr);
    });
  });
  frames.forEach(fr => {
    const box = fr.closest('.overlay-preview-wrap') || fr.parentElement || fr;
    box.__pfFrame = fr;
    ro.observe(box);
  });
})();

// ── Onglet OBS : les 5 options regroupées en sous-menus (pilule) ──
// Une seule section visible à la fois ; masquer une section coupe son aperçu
// (le gestionnaire d'aperçus décharge l'iframe quand sa largeur tombe à 0).
(function () {
  const nav = document.getElementById('obs-subnav');
  if (!nav) return;
  // Rapatrie l'ancien onglet « Stinger » (1er niveau) comme 6e sous-menu OBS.
  (function () {
    const tab = document.getElementById('tab-stinger');
    const host = document.getElementById('tab-custom');
    if (tab && host && !document.getElementById('obs-pane-stinger')) {
      const sect = tab.querySelector('section') || tab;
      sect.classList.add('obs-pane');
      sect.id = 'obs-pane-stinger';
      host.appendChild(sect);
      if (tab !== sect) tab.remove(); // supprime l'enveloppe .tab-content devenue vide
    }
  })();
  const panes = [...document.querySelectorAll('#tab-custom .obs-pane')];
  function showObsPane(id) {
    panes.forEach(p => { p.style.display = (p.id === id) ? '' : 'none'; });
    nav.querySelectorAll('.obs-subnav-btn').forEach(b => b.classList.toggle('active', b.dataset.obspane === id));
    setTimeout(() => window.dispatchEvent(new Event('resize')), 0); // (re)charge / met à l'échelle l'aperçu révélé
  }
  nav.querySelectorAll('.obs-subnav-btn').forEach(btn => {
    btn.addEventListener('click', () => showObsPane(btn.dataset.obspane));
  });
  showObsPane('obs-pane-master'); // sous-menu par défaut
})();

// ── Overlays génériques : aperçu (overlay + master) à droite de chaque panneau ──
// Chaque panneau (cam, cadres, titre, bandeau, minuteur) passe en 2 colonnes :
// réglages à gauche, aperçu de l'overlay + aperçu Master Overlay à droite.
// (Le test A/V est exclu : c'est un utilitaire, pas un overlay composité.)
(function () {
  if (!('ResizeObserver' in window)) return;
  const PANELS = ['accordion-cam', 'accordion-frames', 'accordion-titre', 'accordion-ticker', 'accordion-timer'];

  // Aperçu plein cadre 16:9 mis à l'échelle ; (dé)chargé selon la visibilité du panneau.
  function mountPreview(wrap, url) {
    const fr = document.createElement('iframe');
    fr.style.cssText = 'position:absolute;top:0;left:0;width:1920px;height:1080px;border:none;transform-origin:top left;pointer-events:none';
    wrap.appendChild(fr);
    function scale() { const w = wrap.clientWidth; if (!w) return; const s = w / 1920; fr.style.transform = 'scale(' + s + ')'; wrap.style.height = Math.round(1080 * s) + 'px'; }
    fr.addEventListener('load', () => setTimeout(scale, 50));
    window.addEventListener('resize', scale);
    new ResizeObserver(() => {
      const visible = wrap.getBoundingClientRect().width > 1;
      const src = fr.getAttribute('src');
      if (visible) { if (!src || src === 'about:blank') fr.src = url; scale(); }
      else if (src && src !== 'about:blank') fr.src = 'about:blank';
    }).observe(wrap);
  }
  ...
  PANELS.forEach(id => {
    ...
    const link = panel.querySelector('.accordion-header a[href]');
    const url  = link && link.getAttribute('href');
    if (!body || !url || body.classList.contains('genov-sectioned')) return;
    const baseId = 'genov-' + id.replace('accordion-', '');

    // Masque l'ancien aperçu en ligne (cadres/titre/bandeau) → remplacé par la colonne de droite
    ['frames-preview', 'title-preview', 'ticker-preview'].forEach(pid => {
      const old = body.querySelector('#' + pid);
      if (old) { const w = old.closest('div'); if (w) w.style.display = 'none'; }
    });

    // Regroupe les blocs d'options en sections, délimitées par les titres (h3)
    const blocks = [...body.children];
    const sections = []; let cur = null;
    blocks.forEach(b => {
      const h = b.querySelector('h3');
      if (h && h.textContent.trim()) { cur = { label: h.textContent.trim(), nodes: [] }; sections.push(cur); }
      else if (!cur) { cur = { label: 'Réglages', nodes: [] }; sections.push(cur); }
      cur.nodes.push(b);
    });

    // Nav colorée (vic-nav-title) + zone scrollable avec sections à bordure colorée
    const navT = document.createElement('div'); navT.id = baseId + '-nav-titles'; navT.style.display = 'flex';
    const scroll = document.createElement('div'); scroll.id = baseId + '-scroll-area';
    scroll.style.overflowY = 'auto'; scroll.style.position = 'relative';
    sections.forEach((s, i) => {
      const sectId = baseId + '-sect-' + i;
      const t = document.createElement('span'); t.className = 'vic-nav-title'; t.dataset.target = sectId; t.textContent = s.label;
      navT.appendChild(t);
      const sect = document.createElement('div'); sect.id = sectId;
      s.nodes.forEach(n => sect.appendChild(n));
      scroll.appendChild(sect);
    });

    // Colonne d'aperçus à droite (overlay + master)
    const preview = document.createElement('div');
    preview.className = 'genov-body-preview';
    const mkLabel = t => { const d = document.createElement('div'); d.className = 'genov-pv-label'; d.textContent = t; return d; };
    const mkWrap  = () => { const d = document.createElement('div'); d.className = 'genov-pv-wrap'; return d; };
    const ownWrap = mkWrap(), masterWrap = mkWrap();
    preview.append(mkLabel('Aperçu'), ownWrap, mkLabel('Preview Master Overlay'), masterWrap);

    body.append(navT, scroll, preview);
    body.classList.add('genov-sectioned');
    if (typeof initScrollNav === 'function') initScrollNav(scroll.id, navT.id);
    mountPreview(ownWrap, url);
    mountPreview(masterWrap, '/master');
  });
})();
...
// ── Onglet « Principal » : sous-menus SSBU / Start.gg ───
// (Les overlays génériques sont déplacés dans Customisation › Générique.)
(function () {
  const subnav = document.getElementById('ov-subnav');
  if (!subnav) return;
  const paneIds = ['ov-pane-ssbu', 'ov-pane-startgg'];

  // Déplace le contenu de « Start.gg Overlays » dans le pane Start.gg.
  // Re-parenter des nœuds conserve leurs écouteurs (start.gg, etc.).
  const moveInto = (srcId, dstId) => {
    const src = document.getElementById(srcId), dst = document.getElementById(dstId);
    if (src && dst) { while (src.firstChild) dst.appendChild(src.firstChild); }
  };
  moveInto('tab-startgg',  'ov-pane-startgg');

  subnav.querySelectorAll('.themes-subnav-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      subnav.querySelectorAll('.themes-subnav-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      paneIds.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.display = (id === btn.dataset.ovpane) ? '' : 'none';
      });
      // La sous-nav Match (déplacée dans .match-nav-row) ne concerne que SSBU :
      // on la masque quand le pane Start.gg est actif.
      const ms = document.querySelector('#tab-match .match-subnav');
      if (ms) ms.style.display = (btn.dataset.ovpane === 'ov-pane-ssbu') ? '' : 'none';
      // re-scale les aperçus révélés
      setTimeout(() => window.dispatchEvent(new Event('resize')), 0);
    });
  });
})();

// ── Overlays génériques : navigation horizontale (style SSBU) ───
// Un seul overlay affiché à la fois, sélectionné via la barre d'onglets.
// Handler dédié et auto-scopé (au parent du nav) → pas de conflit avec la
// sous-nav SSBU (qui agit sur #tab-match .match-subpanel).
(function () {
  document.querySelectorAll('.genov-subnav .genov-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const nav = btn.closest('.genov-subnav');
      // La nav peut avoir été déplacée hors du conteneur des panneaux (cf. .match-nav-row
      // de la Customisation) : on cible donc explicitement le conteneur des overlays génériques.
      const container = document.getElementById('cust-cat-generique') || nav.parentElement;
      nav.querySelectorAll('.genov-btn').forEach(b => b.classList.remove('active'));
      container.querySelectorAll('.genov-panel').forEach(p => p.classList.remove('active'));
      btn.classList.add('active');
      const target = container.querySelector('#' + btn.dataset.genpanel);
      if (target) target.classList.add('active');
      // re-scale les aperçus révélés
      setTimeout(() => window.dispatchEvent(new Event('resize')), 0);
    });
...
// ── Onglet « Paramètres » : Connexions (clé start.gg) + Stream (OAuth Twitch/YouTube + overlays) ──
// On relocalise par ID (les écouteurs restent attachés aux nœuds déplacés).
(function () {
  const conn   = document.getElementById('param-connexions');
  const stream = document.getElementById('param-stream');
  const subnav = document.getElementById('param-cat-nav');
  if (!conn || !stream || !subnav) return;

  const moveInto = (id, dst) => { const el = document.getElementById(id); if (el && dst) dst.appendChild(el); };

  // Clé start.gg : déplacée depuis la modale « Configurer le tournoi »
  const keySlot = document.getElementById('param-startgg-key-slot');
  moveInto('setup-key-connected', keySlot);
  moveInto('setup-key-form', keySlot);

  // Stream : connexions plateformes (OAuth Twitch/YouTube) PUIS overlays (Viewers, Chat)
  moveInto('param-sec-twitch-conn', stream);
  moveInto('param-sec-youtube-conn', stream);
  moveInto('param-sec-viewers', stream);
  moveInto('param-sec-twitch-chat', stream);
  moveInto('param-sec-youtube-chat', stream);

  // Bascule des catégories
  subnav.querySelectorAll('.param-cat-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      subnav.querySelectorAll('.param-cat-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      conn.style.display   = (btn.dataset.pcat === 'param-connexions') ? '' : 'none';
      stream.style.display = (btn.dataset.pcat === 'param-stream') ? '' : 'none';
      setTimeout(() => window.dispatchEvent(new Event('resize')), 0);
    });
  });
})();

// ── Onglet « Master Overlay » : compositeur (un seul overlay = plusieurs empilés) ──
(function () {
  ...
  const AVAILABLE = [
    ...
    { id: 'vs-screen', label: 'VS Screen' },
  ];
  const LABEL = Object.fromEntries(AVAILABLE.map(o => [o.id, o.label]));

  // L'ordre affiché = haut de la liste au-dessus. Le serveur empile dans l'ordre
  // du tableau (dernier = z-index le plus haut), donc on inverse à l'envoi.
  let items = []; // ordre d'affichage du master COURANT : [{ id, enabled }]
  let overlays = [];   // liste des master overlays : [{ id, name, items }]
  let currentId = null; // master overlay sélectionné

  function mergeWithAvailable(serverItems) {
    // serverItems sont dans l'ordre serveur (bas → haut) ; on inverse pour l'affichage.
    const fromServer = (serverItems || []).slice().reverse();
    const seen = new Set();
    const merged = [];
    fromServer.forEach(it => { if (LABEL[it.id] && !seen.has(it.id)) { merged.push({ id: it.id, enabled: !!it.enabled }); seen.add(it.id); } });
    AVAILABLE.forEach(o => { if (!seen.has(o.id)) { merged.push({ id: o.id, enabled: false }); seen.add(o.id); } });
    // Les overlays cochés (activés) restent toujours au-dessus des non cochés (tri stable).
    return merged.filter(it => it.enabled).concat(merged.filter(it => !it.enabled));
  }

  // Réordonne en place : activés d'abord, désactivés ensuite (ordre relatif conservé).
  function enabledFirst() {
    items = items.filter(it => it.enabled).concat(items.filter(it => !it.enabled));
  }

  function save() {
    if (!currentId) return;
    // affichage (haut→bas) → serveur (bas→haut)
    const serverItems = items.slice().reverse().map(it => ({ id: it.id, enabled: it.enabled }));
    const cur = overlays.find(o => o.id === currentId);
    if (cur) cur.items = serverItems; // garde la liste locale en phase (compteur, duplication)
    fetch('/api/master/' + encodeURIComponent(currentId), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ items: serverItems }) }).catch(() => {});
  }

  // Overlays dont la customisation peut s'éditer (panneau présent dans Customisation).
  const EDIT_SEL = {
    scoreboard:     '#sb-sub-custom',
    casters:        '#themes-pane-caster > section',
    nextmatch:      '#nm-sub-match',
    'vs-screen':    '#vs-sub-main',
    victory:        '#vic-sub-main',
    cam:            '#accordion-cam',
    frames:         '#accordion-frames',
    ticker:         '#accordion-ticker',
    timer:          '#accordion-timer',
    'stream-title': '#accordion-titre',
  };
  ...
  function render() {
    ...
    items.forEach((it, idx) => {
      const row = document.createElement('div');
      row.style.cssText = 'display:flex;align-items:center;gap:8px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;background:var(--surface2)';
      const lab = document.createElement('label');
      lab.style.cssText = 'display:flex;align-items:center;gap:8px;flex:1;cursor:pointer;min-width:0';
      const cb = document.createElement('input'); cb.type = 'checkbox'; cb.checked = it.enabled;
      const name = document.createElement('span');
      name.textContent = LABEL[it.id] || it.id;
      name.style.cssText = 'overflow:hidden;text-overflow:ellipsis;white-space:nowrap' + (it.enabled ? '' : ';opacity:.45');
      lab.appendChild(cb); lab.appendChild(name);
      const edit = document.createElement('button'); edit.className = 'btn btn-outline btn-sm'; edit.textContent = '✏️'; edit.title = 'Éditer la customisation'; edit.style.padding = '2px 8px';
      if (!EDIT_SEL[it.id]) { edit.disabled = true; edit.style.opacity = '0.3'; edit.title = 'Pas de customisation'; }
      const up = document.createElement('button'); up.className = 'btn btn-outline btn-sm'; up.textContent = '▲'; up.title = 'Monter'; up.style.padding = '2px 8px';
      const down = document.createElement('button'); down.className = 'btn btn-outline btn-sm'; down.textContent = '▼'; down.title = 'Descendre'; down.style.padding = '2px 8px';
      cb.addEventListener('change', () => { it.enabled = cb.checked; enabledFirst(); save(); render(); });
      edit.addEventListener('click', () => { if (EDIT_SEL[it.id]) openEdit(it.id); });
      up.addEventListener('click', () => { if (idx > 0) { [items[idx - 1], items[idx]] = [items[idx], items[idx - 1]]; enabledFirst(); save(); render(); } });
      down.addEventListener('click', () => { if (idx < items.length - 1) { [items[idx + 1], items[idx]] = [items[idx], items[idx + 1]]; enabledFirst(); save(); render(); } });
      row.appendChild(lab); row.appendChild(edit); row.appendChild(up); row.appendChild(down);
      listEl.appendChild(row);
    });
  }

  // ── Popup d'édition : déplace temporairement le panneau de customisation ──
  let _editState = null; // { panel, placeholder }
  function openEdit(id) {
    closeEdit();
    const panel = document.querySelector(EDIT_SEL[id] || '');
    const slot = document.getElementById('master-edit-slot');
    const modal = document.getElementById('master-edit-modal');
    if (!panel || !slot || !modal) return;
    const placeholder = document.createComment('master-edit'); // marque la position d'origine
    panel.parentNode.insertBefore(placeholder, panel);
    slot.appendChild(panel);
    _editState = { panel, placeholder };
    const titleEl = document.getElementById('master-edit-title');
    if (titleEl) titleEl.textContent = 'Édition — ' + (LABEL[id] || id);
    modal.style.display = 'flex';
  ...
  function closeEdit() {
    ...
    if (_editState) {
      const { panel, placeholder } = _editState;
  ...
  document.getElementById('master-edit-close')?.addEventListener('click', closeEdit);
  document.getElementById('master-edit-modal')?.addEventListener('click', e => { if (e.target.id === 'master-edit-modal') closeEdit(); });
  document.addEventListener('keydown', e => { if (e.key === 'Escape' && _editState) closeEdit(); });

  // ── Multi master overlays : sélecteur + CRUD ──────────────────
  const selEl        = document.getElementById('master-select');
  const previewFrame = document.getElementById('preview-frame-master');
  ...
  function renderSelector() {
    if (selEl) {
      ...
      overlays.forEach(o => {
        ...
        opt.value = o.id;
        opt.textContent = o.name + '  (' + (o.items || []).filter(i => i.enabled).length + ')';
  ...
  function updatePreview() {
    if (!previewFrame || !currentId) return;
    const url = '/master/' + encodeURIComponent(currentId);
    previewFrame.dataset.src = url;
    // recharge si déjà chargé (sinon le gestionnaire d'aperçus chargera data-src à l'affichage)
    if (previewFrame.src && !/about:blank$/.test(previewFrame.src)) previewFrame.src = url;
  }

  function loadOverlay(id) {
    const o = overlays.find(x => x.id === id) || overlays[0];
    if (!o) { currentId = null; items = mergeWithAvailable([]); render(); renderSelector(); return; }
    currentId = o.id;
    items = mergeWithAvailable(o.items);
    render();
    renderSelector();
    updatePreview();
  }

  // Sous-menu « Master Overlay » du header (menu Overlays) : un lien /master/<id> par master.
  function renderHeaderMasterMenu() {
    const menu = document.getElementById('overlay-submenu-master');
    if (!menu) return;
    menu.innerHTML = '';
    if (!overlays.length) {
      const e = document.createElement('span');
      e.textContent = '(aucun)';
      e.style.cssText = 'opacity:.5;padding:6px 10px;display:block';
      menu.appendChild(e);
      return;
    }
    overlays.forEach(o => {
      const a = document.createElement('a');
      a.href = '/master/' + encodeURIComponent(o.id);
      a.target = '_blank';
  ...
  function refreshList(selectId) {
    return fetch('/api/master').then(r => r.json()).then(data => {
      overlays = (data && Array.isArray(data.overlays)) ? data.overlays : [];
      renderHeaderMasterMenu();
      loadOverlay(selectId || currentId || (overlays[0] && overlays[0].id));
    }).catch(() => { overlays = []; items = mergeWithAvailable([]); render(); });
  }

  selEl?.addEventListener('change', () => loadOverlay(selEl.value));

  document.getElementById('master-new')?.addEventListener('click', () => {
    const name = prompt('Nom du nouveau master overlay :', 'Master ' + (overlays.length + 1));
    if (name === null) return;
    fetch('/api/master/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim() || ('Master ' + (overlays.length + 1)) }) })
      .then(r => r.json()).then(o => refreshList(o.id)).catch(() => {});
  });

  document.getElementById('master-rename')?.addEventListener('click', () => {
    const cur = overlays.find(o => o.id === currentId); if (!cur) return;
    const name = prompt('Renommer le master overlay :', cur.name);
    if (name === null || !name.trim()) return;
    fetch('/api/master/' + encodeURIComponent(currentId), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim() }) })
      .then(r => r.json()).then(() => refreshList(currentId)).catch(() => {});
  });

  document.getElementById('master-duplicate')?.addEventListener('click', () => {
    const cur = overlays.find(o => o.id === currentId); if (!cur) return;
    fetch('/api/master/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: cur.name + ' (copie)', items: cur.items }) })
      .then(r => r.json()).then(o => refreshList(o.id)).catch(() => {});
  });

  document.getElementById('master-delete')?.addEventListener('click', () => {
    if (overlays.length <= 1) { setStatus('Impossible de supprimer le dernier master overlay'); return; }
    const cur = overlays.find(o => o.id === currentId); if (!cur) return;
    if (!confirm('Supprimer le master overlay « ' + cur.name + ' » ?')) return;
    fetch('/api/master/' + encodeURIComponent(currentId) + '/delete', { method: 'POST' })
      .then(r => r.json()).then(() => { currentId = null; refreshList(); }).catch(() => {});
  });

  refreshList();

  // À l'ouverture de l'onglet : recalcule l'échelle de l'aperçu (qui se (re)charge seul à l'affichage).
  document.querySelector('.tab-btn[data-tab="custom"]')?.addEventListener('click', () => {
...
document.getElementById('center-logo').addEventListener('input', () => {
  updateLogoPreview();
  emitState(buildStateFromForm()); // auto-application (plus de bouton « Appliquer »)
});
  ...
  reader.onload = (ev) => {
    fetch('/api/logo/upload', {
      ...
      .then(({ url }) => {
        ...
        updateLogoPreview();
        emitState(buildStateFromForm()); // auto-application
        e.target.value = '';
      })
...
document.getElementById('sb-overflow-mode')?.addEventListener('change', () => emitState(buildStateFromForm()));
...
// Chaque checkbox émet immédiatement le state ; l'overlay applique body.sb-hide-*.
[
...
// Lots 2 / 3 / 4 : pour chaque paire range/number partageant un id-base, on
...
[
  ...
  'character-size', 'name-font-size', 'tag-font-size', 'pronouns-font-size', 'seed-font-size', 'sb-name-x', 'sb-name-y',
  // Lot 4 : event-bar
  ...
  'event-bar-padding-x', 'event-bar-gap', 'event-bar-bevel', 'event-bar-radius', 'event-bar-detach',
...
// Lot 4 : sb-bg-color-2 (dégradé) + bouton « × » pour le désactiver.
const bg2El = document.getElementById('sb-bg-color-2');
if (bg2El) bg2El.addEventListener('input', () => {
  state.sbBgColor2Active = true;
  emitState(buildStateFromForm());
});
const bg2Clear = document.getElementById('sb-bg-color-2-clear');
if (bg2Clear) bg2Clear.addEventListener('click', () => {
  state.sbBgColor2Active = false;
  emitState(buildStateFromForm());
});

// Lot 3 : couleurs P1/P2 — écrivent dans state.player1.color / state.player2.color.
['p1-accent-color', 'p2-accent-color'].forEach((id, idx) => {
  const el = document.getElementById(id);
  if (!el) return;
  el.addEventListener('input', () => {
    const pNum = idx + 1;
    if (state[`player${pNum}`]) state[`player${pNum}`].color = el.value;
    emitState(buildStateFromForm());
  });
});

// Lot 3 : multi-toggle position du score (entre/au-dessus).
document.querySelectorAll('.score-pos-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.score-pos-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    emitState(buildStateFromForm());
  });
});
...
const SCOREBOARD_DEFAULTS = {
  ...
  characterSize: 100, nameFontSize: 24, tagFontSize: 16, pronounsFontSize: 14, seedFontSize: 11,
  ...
  eventBarPaddingX: 32, eventBarGap: 10, eventBarBevel: 12, eventBarRadius: 0, eventBarDetach: 0,
...
function resetScoreboardCustomization() {
  // 1) Merge defaults into the persistent state (player data preserved)
  Object.assign(state, SCOREBOARD_DEFAULTS);
  state.sbBgColor2Active = false;
  // 2) Reflète chaque default dans l'UI via syncFromState (qui restaure aussi
  //    les sliders, color pickers, multi-toggles, etc.)
  syncFromState({ ...state, ...SCOREBOARD_DEFAULTS });
  // 3) Réactive visuellement la carte preset « Classique » dans la grille Presets
  document.querySelectorAll('.sb-preset-card').forEach(c => c.classList.toggle('active', c.dataset.layout === 'classic'));
  // 4) Émet l'état frais pour propager côté overlay
  emitState(buildStateFromForm());
  setStatus('Scoreboard réinitialisé', 'success');
}

const sbResetBtn = document.getElementById('btn-sb-reset');
if (sbResetBtn) sbResetBtn.addEventListener('click', resetScoreboardCustomization);

function updateCharDisplayModeBtn(mode) {
  const btn = document.getElementById('btn-char-display-mode');
  if (!btn) return;
  // 3 états : normal (par défaut, outline) | mural (warning, surligné) | off (danger, alerte rouge)
  const labels  = { normal: '🖼 Personnages : Normal', mural: '🖼 Personnages : Mural', off: '🚫 Personnages : Désactivés' };
  btn.textContent = labels[mode] || labels.normal;
  btn.classList.toggle('btn-outline', mode === 'normal');
  btn.classList.toggle('btn-warning', mode === 'mural');
  btn.classList.toggle('btn-danger',  mode === 'off');
  // Classe sur <body> : permet de cacher en CSS les options liées aux personnages
  // (sélecteur perso + couleurs stock icon) dans l'onglet Scoreboard.
  document.body.classList.toggle('char-off', mode === 'off');
}

document.getElementById('btn-char-display-mode').addEventListener('click', () => {
  // Cycle : normal → mural → off → normal
  const next = { normal: 'mural', mural: 'off', off: 'normal' };
  state.charDisplayMode = next[state.charDisplayMode] || 'mural';
  updateCharDisplayModeBtn(state.charDisplayMode);
  emitState(buildStateFromForm());
  const statusMsg = {
    normal: 'Affichage normal activé',
    mural:  'Affichage mural activé',
    off:    'Personnages désactivés',
  };
...
// Activer / désactiver le logo central (garde l'URL ; masqué = retour au « VS »)
document.getElementById('btn-logo-toggle')?.addEventListener('click', function () {
  const on = !this.classList.contains('active');
  this.classList.toggle('active', on);
  this.textContent = on ? '🖼 Logo central : Affiché' : '🖼 Logo central : Masqué';
  emitState(buildStateFromForm());
  setStatus(on ? 'Logo central affiché' : 'Logo central masqué (VS)');
});
...
(function () {
  ...
  document.querySelectorAll('.overlay-preview-frame').forEach(frame => {
    ...
    if (!wrap) return;
    const rescale = () => scalePreviewWrap(wrap);
      ...
      try {
        ...
        if (doc && doc.body) {
          const mo = new MutationObserver(() => {
            clearTimeout(frame._rescaleT);
            frame._rescaleT = setTimeout(rescale, 150);
          });
...
let castersState = {
  ...
  // Position + style (vides/0 = utiliser le style par défaut de l'overlay)
  posX: 0, posY: 0,
  socialSize: 0, socialColor: '',
  cardRadius: '', cardPadding: '', cardBorderWidth: '', cardBorderColor: '#2a2a3a',
};
...
function syncCastersFromState(s) {
  ...
  // Position
  setNum('cb-posX', s.posX, 0);
  setNum('cb-posY', s.posY, 0);
  // Réseaux
  setNum('cb-socialSize', s.socialSize || '', '');
  const scOn = document.getElementById('cb-socialColorOn');
  if (scOn) scOn.checked = !!s.socialColor;
  setCol('cb-socialColor', s.socialColor, '#8899BB');
  // Cartes
  setNum('cb-cardRadius',  s.cardRadius, '');
  setNum('cb-cardPadding', s.cardPadding, '');
  const cbOn = document.getElementById('cb-cardBorderOn');
  if (cbOn) cbOn.checked = !!s.cardBorderWidth;
  setNum('cb-cardBorderWidth', s.cardBorderWidth || '', '');
  setCol('cb-cardBorderColor', s.cardBorderColor, '#2a2a3a');
}
...
// ── Builder casters ───────────────────────────────────────────
{
  ...
  const builderFields = [
    ...
    // Position + style (int autorise 0 et négatifs)
    { id: 'cb-posX',        key: 'posX',        type: 'int' },
    { id: 'cb-posY',        key: 'posY',        type: 'int' },
    { id: 'cb-socialSize',  key: 'socialSize',  type: 'int' },
    { id: 'cb-cardRadius',  key: 'cardRadius',  type: 'int' },
    { id: 'cb-cardPadding', key: 'cardPadding', type: 'int' },
  ];
  builderFields.forEach(({ id, key, type }) => {
    ...
    el.addEventListener('input', () => {
      ...
      else if (type === 'int') { const v = parseInt(el.value); castersState[key] = Number.isNaN(v) ? 0 : v; }
      else castersState[key] = el.value;
      emitCasters();
    });
  });

  // Couleur des réseaux + bordure des cartes : activées par une case à cocher
  // (sinon on garde le style par défaut de l'overlay : couleurs par réseau, bordure CSS).
  function applyCasterStyleToggles() {
    const scOn = document.getElementById('cb-socialColorOn');
    castersState.socialColor = (scOn && scOn.checked)
      ? (document.getElementById('cb-socialColor')?.value || '') : '';
    const cbOn = document.getElementById('cb-cardBorderOn');
    if (cbOn && cbOn.checked) {
      castersState.cardBorderWidth = parseInt(document.getElementById('cb-cardBorderWidth')?.value) || 0;
      castersState.cardBorderColor = document.getElementById('cb-cardBorderColor')?.value || '#2a2a3a';
    } else {
      castersState.cardBorderWidth = '';
    }
    emitCasters();
  }
  ['cb-socialColorOn', 'cb-socialColor', 'cb-cardBorderOn', 'cb-cardBorderWidth', 'cb-cardBorderColor']
    .forEach(id => { const el = document.getElementById(id); if (el) el.addEventListener('input', applyCasterStyleToggles); });
}
...
// ── Charger un caster : popup à 3 sources (enregistrés / tournoi / start.gg) ──
let _casterLoadSlot = 1;

function casterFill(slot, data) {
...
function _clRow(name, sub, onClick, seed) {
  const row = document.createElement('div');
  row.className = 'caster-load-row';
  const info = document.createElement('div');
  const nameEl = document.createElement('div'); nameEl.className = 'cl-name'; nameEl.textContent = name || '(sans nom)';
  info.appendChild(nameEl);
  if (sub) { const subEl = document.createElement('div'); subEl.className = 'cl-sub'; subEl.textContent = sub; info.appendChild(subEl); }
  row.appendChild(info);
  if (seed) { const s = document.createElement('div'); s.className = 'cl-seed'; s.textContent = '#' + seed; row.appendChild(s); }
  row.addEventListener('click', onClick);
  return row;
}

function _clRenderSaved() {
  const el = document.getElementById('caster-load-saved-list');
  if (!el) return;
  const list = rosterLoad();
  el.innerHTML = '';
  if (!list.length) { el.innerHTML = '<div class="caster-load-empty">Aucun caster enregistré.</div>'; return; }
  list.forEach(c => {
    const sub = [c.twitter, c.twitch, c.youtube].filter(Boolean).join(' · ');
    el.appendChild(_clRow(c.name, sub, () => { casterFill(_casterLoadSlot, c); closeCasterLoadPopup(); setStatus('Caster chargé : ' + (c.name || '')); }));
  });
}

function _clRenderTournament(filter) {
  const el = document.getElementById('caster-load-tour-list');
  if (!el) return;
  const entrants = (window.sggGetEntrants ? window.sggGetEntrants() : []) || [];
  el.innerHTML = '';
  if (!entrants.length) { el.innerHTML = '<div class="caster-load-empty">Aucun participant chargé (configure/charge un tournoi).</div>'; return; }
  const f = (filter || '').toLowerCase();
  const filtered = entrants.filter(e => !f || (e.tag || e.name || '').toLowerCase().includes(f));
  if (!filtered.length) { el.innerHTML = '<div class="caster-load-empty">Aucun joueur.</div>'; return; }
  filtered.forEach(e => {
    el.appendChild(_clRow(e.tag || e.name, e.prefix || '', async () => {
      const data = { name: e.tag || e.name || '', twitter: '', twitch: '', youtube: '' };
      if (e.playerId) {
        try {
          const s = await fetch('/api/startgg/player-socials/' + e.playerId).then(r => r.json());
          if (!s.error) { data.twitter = s.twitter || ''; data.twitch = s.twitch || ''; }
        } catch (err) {}
      }
      casterFill(_casterLoadSlot, data);
      closeCasterLoadPopup();
      setStatus('Caster chargé : ' + data.name);
    }, e.seeding));
  });
}

function _clParseSlug(input) {
  let s = (input || '').trim();
  const m = s.match(/start\.gg\/user\/([^/?#]+)/i);
  if (m) return m[1];
...
async function _clSearch() {
  const input = document.getElementById('caster-load-search-input');
  const out = document.getElementById('caster-load-search-result');
  if (!input || !out) return;
  const slug = _clParseSlug(input.value);
  if (!slug) { out.innerHTML = '<div class="caster-load-empty">Entre un lien ou un slug.</div>'; return; }
  out.innerHTML = '<div class="caster-load-empty">Recherche…</div>';
  try {
    const r = await fetch('/api/startgg/user/' + encodeURIComponent(slug)).then(x => x.json());
    if (r.error || !r.name) { out.innerHTML = '<div class="caster-load-empty">Joueur introuvable.</div>'; return; }
    out.innerHTML = '';
    const sub = [r.twitter, r.twitch].filter(Boolean).join(' · ');
    out.appendChild(_clRow(r.name, sub, () => { casterFill(_casterLoadSlot, r); closeCasterLoadPopup(); setStatus('Caster chargé : ' + r.name); }));
  } catch (e) { out.innerHTML = '<div class="caster-load-empty">Erreur : ' + e.message + '</div>'; }
}

function openCasterLoadPopup(slot) {
  _casterLoadSlot = slot;
  const modal = document.getElementById('caster-load-modal');
  if (!modal) return;
  document.getElementById('caster-load-title').textContent = 'Charger un caster — C' + slot;
  modal.querySelectorAll('.themes-subnav-btn').forEach(b => b.classList.toggle('active', b.dataset.cltab === 'saved'));
  modal.querySelectorAll('[data-clpane]').forEach(p => { p.style.display = (p.dataset.clpane === 'saved') ? '' : 'none'; });
  _clRenderSaved();
  const ts = document.getElementById('caster-load-tour-search'); if (ts) ts.value = '';
  const si = document.getElementById('caster-load-search-input'); if (si) si.value = '';
  const sr = document.getElementById('caster-load-search-result'); if (sr) sr.innerHTML = '';
  modal.style.display = 'flex';
}
function closeCasterLoadPopup() {
  const modal = document.getElementById('caster-load-modal');
  if (modal) modal.style.display = 'none';
}

document.querySelectorAll('#caster-load-modal .themes-subnav-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    const modal = document.getElementById('caster-load-modal');
    modal.querySelectorAll('.themes-subnav-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    const tab = btn.dataset.cltab;
    modal.querySelectorAll('[data-clpane]').forEach(p => { p.style.display = (p.dataset.clpane === tab) ? '' : 'none'; });
    if (tab === 'tournament') _clRenderTournament(document.getElementById('caster-load-tour-search')?.value);
  });
});
document.getElementById('caster-load-tour-search')?.addEventListener('input', e => _clRenderTournament(e.target.value));
document.getElementById('caster-load-search-btn')?.addEventListener('click', _clSearch);
document.getElementById('caster-load-search-input')?.addEventListener('keydown', e => { if (e.key === 'Enter') _clSearch(); });
document.getElementById('caster-load-close')?.addEventListener('click', closeCasterLoadPopup);
document.getElementById('caster-load-modal')?.addEventListener('click', e => { if (e.target.id === 'caster-load-modal') closeCasterLoadPopup(); });
document.addEventListener('keydown', e => { if (e.key === 'Escape' && document.getElementById('caster-load-modal')?.style.display === 'flex') closeCasterLoadPopup(); });

document.getElementById('btn-load-roster-1')?.addEventListener('click', function (e) { e.stopPropagation(); openCasterLoadPopup(1); });
document.getElementById('btn-load-roster-2')?.addEventListener('click', function (e) { e.stopPropagation(); openCasterLoadPopup(2); });
...
// ── Thème : barre de types (même format que les autres menus de config :
//    nav colorée en haut + zone scrollable avec sections, via initScrollNav) ──
(function () {
  const grid = document.getElementById('themes-grid');
  if (!grid) return;

  // Regrouper les enfants de la grille par catégorie (libellés .themes-group-label)
  const cats = [];
  let cur = null;
  [...grid.children].forEach(child => {
    if (child.classList.contains('themes-group-label')) {
      cur = { name: child.textContent.trim(), nodes: [] };
      cats.push(cur);
    } else if (cur) {
  ...
  // « custom » seul dans sa propre catégorie « Custom », tout en haut (séparé de Spécial)
  if (cats[0]) {
    const ci = cats[0].nodes.findIndex(n => n.dataset && n.dataset.theme === 'custom');
    if (ci >= 0) cats.unshift({ name: 'Custom', nodes: [cats[0].nodes.splice(ci, 1)[0]] });
  }

  // Conteneur : nav en haut (pleine largeur), zone scrollable en dessous
  const container = document.createElement('div');
  container.id = 'themes-cust-grid';

  const navT = document.createElement('div');
  navT.id = 'themes-nav-titles';
  navT.style.display = 'flex';

  const scroll = document.createElement('div');
  scroll.id = 'themes-scroll-area';
  scroll.style.overflowY = 'auto';
  scroll.style.position = 'relative';

  cats.forEach((c, i) => {
    const sectId = 'themes-sect-' + i;
    const title = document.createElement('span');
    title.className = 'vic-nav-title';
    title.dataset.target = sectId;
    title.textContent = c.name;
    navT.appendChild(title);

    const sect = document.createElement('div');
    sect.id = sectId;

    // Tête de section : titre + flèche « Voir tout » (visible seulement si la ligne déborde)
    const head = document.createElement('div');
    head.className = 'themes-sect-head';
    const heading = document.createElement('h3');
    heading.className = 'themes-sect-title';
    heading.textContent = c.name;
    head.appendChild(heading);
    const moreBtn = document.createElement('button');
    moreBtn.type = 'button';
    moreBtn.className = 'themes-sect-more';
    moreBtn.textContent = 'Voir tout ›';
    moreBtn.style.display = 'none';
    moreBtn.addEventListener('click', () => openCatModal(sectId, c.name));
    head.appendChild(moreBtn);
    sect.appendChild(head);

    const inner = document.createElement('div');
    inner.className = 'themes-grid';
    c.nodes.forEach(n => inner.appendChild(n)); // déplace les cartes existantes (handlers conservés)
    sect.appendChild(inner);
    scroll.appendChild(sect);
  });

  container.appendChild(navT);
  container.appendChild(scroll);
  grid.parentNode.insertBefore(container, grid);
  grid.remove(); // supprime l'ancienne grille + les libellés (remplacés par la nav colorée)

  if (typeof initScrollNav === 'function') initScrollNav('themes-scroll-area', 'themes-nav-titles');

  // ── Popup « catégorie complète » : ouvert par la flèche quand une ligne déborde ──
  const catModal = document.createElement('div');
  catModal.id = 'themes-cat-modal';
  catModal.innerHTML =
    '<div class="themes-cat-modal-box">' +
      '<div class="themes-cat-modal-head">' +
        '<h3 id="themes-cat-modal-title" class="themes-sect-title"></h3>' +
        '<input type="text" id="themes-cat-modal-search" placeholder="Rechercher un thème…" autocomplete="off">' +
        '<button type="button" id="themes-cat-modal-close" aria-label="Fermer">✕</button>' +
      '</div>' +
      '<div id="themes-cat-modal-body" class="themes-grid"></div>' +
    '</div>';
  document.body.appendChild(catModal);
  const modalBody   = document.getElementById('themes-cat-modal-body');
  const modalTitle  = document.getElementById('themes-cat-modal-title');
  const modalSearch = document.getElementById('themes-cat-modal-search');

  // Recherche clavier : filtre les cartes (insensible à la casse ET aux accents)
  const _norm = s => (s || '').normalize('NFD').replace(/\p{Diacritic}/gu, '').toLowerCase();
  function filterCatModal() {
    const q = _norm(modalSearch.value.trim());
    modalBody.querySelectorAll('.theme-preset-card').forEach(card => {
      card.style.display = (!q || _norm(card.textContent).includes(q)) ? '' : 'none';
    });
  }
  modalSearch.addEventListener('input', filterCatModal);

  function openCatModal(sectId, name) {
    const sect = document.getElementById(sectId);
    if (!sect) return;
    modalTitle.textContent = name;
    modalBody.innerHTML = '';
    // Clones (sans id pour éviter les doublons) ; clic délégué → applyTheme
    sect.querySelectorAll('.theme-preset-card').forEach(card => {
    ...
    requestAnimationFrame(() => modalSearch.focus()); // recherche au clavier dès l'ouverture
  }
  function closeCatModal() { catModal.classList.remove('open'); modalBody.innerHTML = ''; }

  catModal.addEventListener('click', e => {
    if (e.target === catModal || e.target.closest('#themes-cat-modal-close')) { closeCatModal(); return; }
    const card = e.target.closest('.theme-preset-card');
    if (card && card.dataset.theme) applyTheme(card.dataset.theme); // les clones reflètent l'état .active
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && catModal.classList.contains('open')) closeCatModal();
  });

  // ── Une seule ligne par catégorie : la flèche n'apparaît que si le contenu déborde ──
  function updateThemeArrows() {
    scroll.querySelectorAll('[id^="themes-sect-"]').forEach(sect => {
      const g = sect.querySelector('.themes-grid');
      const more = sect.querySelector('.themes-sect-more');
      if (!g || !more) return;
      more.style.display = (g.scrollWidth - g.clientWidth > 2) ? '' : 'none';
    });
  }
  window.addEventListener('resize', updateThemeArrows);
  if ('ResizeObserver' in window) new ResizeObserver(updateThemeArrows).observe(scroll);
  setTimeout(updateThemeArrows, 400);

  // ── Carte « Créer un thème custom » (en tête de Spécial) → popup d'édition de thème ──
  (function () {
    const customCard = scroll.querySelector('.theme-preset-card[data-theme="custom"]');
    if (!customCard) return;

    // Renomme la carte + la place en tête de la section Spécial
    const nameEl = customCard.querySelector('.theme-preset-name');
    if (nameEl) {
      const badge = document.getElementById('theme-card-custom-badge');
      nameEl.textContent = 'Créer un thème custom';
      if (badge) nameEl.appendChild(badge);
    }
    const sect0grid = document.querySelector('#themes-sect-0 .themes-grid');
    if (sect0grid && sect0grid.firstChild !== customCard) sect0grid.insertBefore(customCard, sect0grid.firstChild);

    // Popup : on y déplace les vrais réglages de thème (Fond + Style), restitués à la fermeture
    const cm = document.createElement('div');
    cm.id = 'themes-custom-modal';
    cm.innerHTML =
      '<div class="themes-cat-modal-box">' +
        '<div class="themes-cat-modal-head">' +
    ...
    const fontSect = document.createElement('div');
    fontSect.style.padding = '0 16px 16px';
    fontSect.innerHTML =
      '<div class="custom-card"><h3>Police</h3>' +
      '<select id="theme-custom-font" style="width:100%;background:var(--surface2);border:1px solid var(--border);border-radius:4px;color:var(--text);font-size:13px;padding:6px 8px">' +
    ...
    // Capture + stopImmediatePropagation : empêche le applyTheme générique, on ouvre juste le popup
    customCard.addEventListener('click', e => { e.stopImmediatePropagation(); openCustomModal(); }, true);
...
// ── Remplir les casters avec des exemples (aperçu) / vider ─────
document.getElementById('btn-fill-caster-placeholders')?.addEventListener('click', () => {
  const set = (id, v) => { const el = document.getElementById(id); if (el) { el.value = v; el.dispatchEvent(new Event('input', { bubbles: true })); } };
  set('c1-name', 'CASTER UN');   set('c1-twitter', '@caster_un');   set('c1-twitch', 'caster_un');   set('c1-youtube', '@caster_un');
  set('c2-name', 'CASTER DEUX');  set('c2-twitter', '@caster_deux'); set('c2-twitch', 'caster_deux'); set('c2-youtube', '@caster_deux');
  const ns = buildCastersFromForm();
  ns.visible = true;
  socket.emit('updateCasters', ns);
  castersState = ns;
  const vb = document.getElementById('btn-casters-visibility');
  if (vb) { vb.textContent = 'Masquer'; vb.style.background = '#1a3a2a'; vb.style.borderColor = 'var(--gold)'; }
  setStatus('Casters remplis avec des exemples (aperçu)');
});

document.getElementById('btn-clear-casters')?.addEventListener('click', () => {
  ['c1-name', 'c1-twitter', 'c1-twitch', 'c1-youtube', 'c2-name', 'c2-twitter', 'c2-twitch', 'c2-youtube']
    .forEach(id => { const el = document.getElementById(id); if (el) { el.value = ''; el.dispatchEvent(new Event('input', { bubbles: true })); } });
  const ns = buildCastersFromForm();
  socket.emit('updateCasters', ns);
  castersState = ns;
  setStatus('Champs casters vidés');
});
...
socket.on('vsScreenTrigger', () => updateVsToggleBtn(true));
socket.on('vsScreenHide', () => updateVsToggleBtn(false));
socket.on('transitionsUpdate', data => {
  if (data.victory !== undefined) updateVictoryToggleBtn(!!data.victory.visible);
  if (data['vs-screen'] !== undefined) updateVsToggleBtn(!!data['vs-screen'].visible);
});
...
(function () {
  ...
  function getDynamicValue(key) {
    ...
    const map = {
      ...
      'event':       () => s.event  || '',
      'stage':       () => s.stage  || '',
      'format':      () => s.format === 'custom' ? (s.customWins || 'Custom') : (s.format || ''),
    };
...
(function () {
  ...
  // Ouverture du sous-onglet « Caster » du Builder
  document.querySelectorAll('.builder-subnav-btn[data-bpane="builder-pane-caster"]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      if(layouts.length===0&&!activeId) init();
      else{updateScale();renderCanvas();renderLayers();}
    });
...
(function () {
  ...
  if (!toggleBtn) return; // Mode Régie retiré → module désactivé proprement
...
// ── Notes : rapatriées dans « Principal › Casters » (sous le panneau de réglages) ──
// L'ancien onglet « Notes » de 1er niveau est supprimé ; son panneau devient une
// section du sous-panneau Casters. Le script Notes ci-dessous retrouve ses éléments
// par id (ils sont déplacés, pas recréés).
(function () {
  const notesTab = document.getElementById('tab-notes');
  const casters  = document.getElementById('casters-sub-match');
  if (!notesTab || !casters || document.getElementById('casters-notes-section')) return;
  const panel = notesTab.querySelector('.notes-panel') || notesTab;
  const sect = document.createElement('section');
  sect.className = 'section';
  sect.id = 'casters-notes-section';
  sect.innerHTML = '<div class="section-header"><h2>Notes</h2></div>';
  sect.appendChild(panel);
  casters.appendChild(sect);
  notesTab.remove(); // supprime l'enveloppe .tab-content devenue vide
})();
...
(function () {
  ...
  socket.on('tournamentConfigUpdate', cfg => {
    ...
    // Recharger les données start.gg dès qu'un tournoi est configuré (bracket choisi ou non)
    if (cfg.slug) window.sggAutoLoad?.();
  });
...
// ── S3 : Accès distant ngrok (Paramètres › Connexions) ──────────────────────
(function () {
  const connectedBlock = document.getElementById('ngrok-connected');
  const formBlock      = document.getElementById('ngrok-form');
  if (!connectedBlock || !formBlock) return;   // panel ngrok absent → no-op

  const urlCode        = document.getElementById('ngrok-url');
  const urlCopyBtn     = document.getElementById('ngrok-url-copy');
  const disconnectBtn  = document.getElementById('ngrok-disconnect');
  const authInput      = document.getElementById('ngrok-authtoken');
  const connectBtn     = document.getElementById('ngrok-connect');
  const statusLine     = document.getElementById('ngrok-status');
  const domainInput    = document.getElementById('ngrok-domain');
  const domainSaveBtn  = document.getElementById('ngrok-domain-save');
  const autostartCheck = document.getElementById('ngrok-autostart');

  function setAccessStatus(msg, cls) {
    statusLine.textContent = msg || '';
    if (cls === 'ok')       statusLine.style.color = '#6BC96C';
    else if (cls === 'err') statusLine.style.color = 'var(--danger)';
    else                    statusLine.style.color = 'var(--text-muted)';
  }

  function applyAccessState(status) {
    const active = !!(status && status.active);
    connectedBlock.style.display = active ? 'flex' : 'none';
    formBlock.style.display      = active ? 'none' : '';
    if (active) {
      urlCode.textContent       = status.publicUrl || '';
      urlCopyBtn.dataset.copy   = status.publicUrl || '';
      authInput.value           = '';   // ne jamais laisser le token en mémoire DOM
    }
    if (status) {
      // Ne pas écraser ce que l'utilisateur tape dans le champ domaine
      if (document.activeElement !== domainInput) domainInput.value = status.domain || '';
      autostartCheck.checked = !!status.autostart;
    }
    if (status && status.lastError) {
      setAccessStatus(status.lastError, 'err');
    } else if (active) {
      setAccessStatus('Tunnel en ligne.', 'ok');
    } else if (status && status.status === 'connecting') {
      setAccessStatus('Connexion en cours…', '');
    } else if (status && status.configured) {
      setAccessStatus('Authtoken enregistré. Cliquez sur Connecter.', '');
    } else {
      setAccessStatus('', '');
    }
  }

  // ─── Bouton Connecter : enregistre token (+ domaine si tapé) puis démarre ───
  connectBtn.addEventListener('click', async () => {
    const token = authInput.value.trim();
    if (!token) { setAccessStatus('Veuillez saisir un authtoken.', 'err'); return; }
    connectBtn.disabled = true;
    setAccessStatus('Enregistrement…', '');
    try {
      const payload = { authtoken: token };
      const dom = domainInput.value.trim();
      if (dom) payload.domain = dom;
      await fetch('/api/remote-access/config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      setAccessStatus('Démarrage du tunnel…', '');
      const res = await fetch('/api/remote-access/start', { method: 'POST' });
      const status = await res.json();
      applyAccessState(status);
    } catch (err) {
      setAccessStatus('Erreur réseau.', 'err');
    } finally {
  ...
  // ─── Enregistrer le domaine (sans toucher au tunnel actif) ───
  domainSaveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const status = await res.json();
      applyAccessState(status);
      setAccessStatus(dom ? 'Domaine enregistré.' : 'Domaine effacé.', 'ok');
    } catch { setAccessStatus('Erreur réseau.', 'err'); }
    finally { domainSaveBtn.disabled = false; }
  });

  // ─── Checkbox autostart : POST indépendant à chaque toggle ───
  autostartCheck.addEventListener('change', async () => {
    try {
      await fetch('/api/remote-access/config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ autostart: autostartCheck.checked }),
      });
    } catch {
      autostartCheck.checked = !autostartCheck.checked;   // rollback visuel
      setAccessStatus('Erreur réseau.', 'err');
    }
  });

  // ─── Init au boot : récupère l'état du tunnel et l'affiche ───
  fetch('/api/remote-access/status')
    .then(r => r.json())
    .then(applyAccessState)
    .catch(() => setAccessStatus('Impossible de charger l\'état du tunnel.', 'err'));

  // ─── Sync live via Socket.IO ───
  if (typeof socket !== 'undefined' && socket && socket.on) {
    socket.on('remoteAccessUpdate', applyAccessState);
  }
})();
...
// ═══════════════════════════════════════════════════════════════
// INDICATEUR TOURNOI — alerte « participants non chargés »
// Un tournoi peut être configuré (nom/logo persistés) sans que les participants
// start.gg soient chargés en mémoire (typiquement après un redémarrage du serveur).
// On bascule alors la classe .sgg-not-loaded sur l'indicateur (pastille + bord orange).
(function () {
  const indicator = document.getElementById('tournament-indicator');
  if (!indicator) return;
  const baseTitle = indicator.getAttribute('title') || 'Configuration du tournoi';

  function refresh() {
    const configured = indicator.classList.contains('configured');
    const loaded = ((window.sggGetEntrants && window.sggGetEntrants()) || []).length > 0;
    const notLoaded = configured && !loaded;
    indicator.classList.toggle('sgg-not-loaded', notLoaded);
    indicator.setAttribute('title', notLoaded
      ? 'Participants start.gg non chargés — recharge-les (Principal ▸ Match depuis start.gg)'
      : baseTitle);
  }

  // Mise à jour instantanée au chargement des participants (on préserve un hook éventuel).
  const prevHook = window.onStartggEntrantsLoaded;
  window.onStartggEntrantsLoaded = function () {
    try { if (typeof prevHook === 'function') prevHook.apply(this, arguments); }
    finally { refresh(); }
  };

  // Filet de sécurité (reflète aussi config changée / autres chemins de chargement).
  setInterval(refresh, 1500);
})();

// ── Scoreboard (onglet Principal) : zoom auto pour occuper la hauteur dispo ──
// Largeur plafonnée (CSS max-width) → marge horizontale pour pouvoir AGRANDIR ;
// sinon RÉTRÉCIT pour toujours tenir sans scroll. Origine du scale : top center.
// On compense la mise en page via margin-bottom (le transform ne réserve pas d'espace).
(function () {
  const grid = document.querySelector('#tab-match .scoreboard-grid');
  const tabMatch = document.getElementById('tab-match');
  const sub = document.getElementById('sb-sub-match');
  if (!grid || !tabMatch || !sub) return;
  const pso = document.getElementById('pso-footer');
  const app = document.querySelector('.app');
  let topWithNavs = null;   // position du scoreboard quand les navs sont visibles (→ échelle stable)

  function fitScoreboard() {
    const canFit = !document.body.classList.contains('mobile-mode')
                 && tabMatch.classList.contains('active')
                 && sub.classList.contains('active')
                 && grid.offsetParent !== null;
    if (!canFit) {                                   // hors desktop/scoreboard : on neutralise le zoom
      if (grid.style.transform && grid.style.transform !== 'none') { grid.style.transform = 'none'; grid.style.marginTop = '0px'; grid.style.marginBottom = '0px'; }
      return;
    }
    // mesurer la taille naturelle (sans transform ni compensation)
    ...
    const actualTop = grid.getBoundingClientRect().top + window.scrollY;   // position réelle (état navs courant)
    // Échelle STABLE au toggle des navs (« modifier uniquement la hauteur ») : pour le CALCUL
    // d'échelle on réserve toujours l'espace des navs (position mémorisée quand elles sont visibles).
    const navsHidden = app && app.classList.contains('navs-hidden');
    if (!navsHidden) topWithNavs = actualTop;
    const refTop = (navsHidden && topWithNavs != null) ? topWithNavs : actualTop;
    const footerH = pso ? pso.offsetHeight : 0;
    const reserve = footerH + 44;                            // padding section + footer + marge
    // Échelle : suit la hauteur dispo (navs réservées → reste stable au toggle)
    let k = (window.innerHeight - refTop - reserve) / naturalH;
    const maxW = grid.parentElement.clientWidth - 120;       // marge latérale (anti-débordement / glows)
    if (k > 1 && naturalW * k > maxW) k = Math.max(1, maxW / naturalW);
    k = Math.max(0.5, Math.min(k, 1.8));                     // bornes de sécurité
    // Centrage vertical dans la zone visible sous les navs (footer hors-écran accepté).
    // marginTop = moitié de l'espace libre ; la compensation marginBottom fait suivre le footer.
    const visualH = naturalH * k;
    const centerRegion = window.innerHeight - actualTop - 20;
    const marginTop = Math.max(0, Math.round((centerRegion - visualH) / 2));
    grid.style.marginTop = marginTop + 'px';
    grid.style.marginBottom = Math.round(naturalH * (k - 1)) + 'px';
    // Centrage HORIZONTAL précis : on mesure le centre réel puis on corrige avec translateX
    // pour viser le centre visible du viewport (robuste vs scrollbar / décalage de conteneur).
    grid.style.transform = 'scale(' + k + ')';
    const rr = grid.getBoundingClientRect();
    const dx = Math.round(document.documentElement.clientWidth / 2 - (rr.left + rr.right) / 2);
    grid.style.transform = 'translateX(' + dx + 'px) scale(' + k + ')';
  }
  window.__fitScoreboard = fitScoreboard;

  let ticking = false;
  const schedule = () => { if (ticking) return; ticking = true; requestAnimationFrame(() => { ticking = false; fitScoreboard(); }); };
  window.addEventListener('resize', schedule);
  // re-fit quand le contenu de la grille change de taille (perso choisi, format custom…)
  if (window.ResizeObserver) { try { new ResizeObserver(schedule).observe(grid); } catch (e) {} }
  fitScoreboard();
